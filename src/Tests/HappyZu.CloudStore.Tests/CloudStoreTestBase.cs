...
namespace HappyZu.CloudStore.Tests
{
    public abstract class CloudStoreTestBase : AbpIntegratedTestBase<CloudStoreTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CloudStoreTestBase()
        {
        ...
        protected void UsingDbContext(Action<CloudStoreDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<CloudStoreDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CloudStoreDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CloudStoreDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CloudStoreDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CloudStoreDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<CloudStoreDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CloudStoreDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CloudStoreDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CloudStoreDbContext, Task<T>> func)
        {
            T result;
